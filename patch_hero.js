const fs = require('fs');
let content = fs.readFileSync('src/components/landing/HeroSection.jsx', 'utf8');

// Update font sizing
content = content.replace(
  'h1 className="mt-8 text-5xl font-extrabold leading-[1.1]',
  'h1 className="mt-12 text-5xl font-extrabold leading-[1.1] md:text-7xl"'
);
content = content.replace(
  'text-brand-navy md:text-6xl tracking-tight',
  'text-[#0a1b2d] tracking-tight'
);
...
content = content.replace(
  ...
  'absolute right-0 xl:-right-10 top-20 z-20 w-[300px] rounded-2xl bg-[#131f33]'
);

fs.writeFileSync('src/components/landing/HeroSection.jsx', content);
