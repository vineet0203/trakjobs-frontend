...
const trustHighlights = [
  { title: "Residential &\nCommercial Services", icon: Building2 },
  { title: "Skilled & Background\nChecked Pros", icon: ShieldCheck },
  { title: "On-Time\nGuaranteed", icon: Clock },
  { title: "Satisfaction\n100% Guaranteed", icon: Award },
];
    ...
    <section id="home" className="relative min-h-[600px] overflow-hidden bg-[#fafafa]">
      <div className="mx-auto w-full max-w-none px-6 md:px-10 lg:px-14 pt-12 lg:pt-16 pb-12">
        <div className="flex flex-col gap-10 lg:flex-row lg:items-stretch">
          ...
          >
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-1.5 text-xs font-semibold text-slate-700 shadow-sm">
          ...
          <motion.div
            ...
            className="relative lg:flex-[0_0_55%] lg:max-w-[55%] min-h-[560px] hidden lg:block"
          >
            <div className="absolute inset-0 overflow-hidden after:pointer-events-none after:absolute after:inset-0 after:bg-[linear-gradient(to_bottom,transparent_65%,#fafafa_100%)]">
              <img
                src={landingImage}
                alt="Handyman"
                className="h-full w-full object-cover object-top" style={{transform: "translateX(-20%)"}}
              />
            </div>

            {/* Floating Card */}
            <div className="absolute right-0 top-4 z-20 w-[200px] rounded-2xl bg-[#1a2940] px-5 py-6 text-white shadow-2xl">
              <div className="flex flex-col gap-6">
                {trustHighlights.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    ...
                    <span className="text-[15px] font-medium leading-snug whitespace-pre-line">
                      {item.title}
                    </span>
                  </div>
                ))}
